...
from medlineplus_api import fetch_medlineplus, parse_medlineplus 
...
def load_pdfs(data_dir: Path):
    """Loads every PDF in data_dir and returns one LangChain Document per page."""
...
def load_medlineplus():
    """Fetches hypertension information from MedlinePlus API."""
    
    print("\nFetching MedlinePlus data...")

    xml_data = fetch_medlineplus("hypertension")

    documents = parse_medlineplus(xml_data)

    print(f"  -> {len(documents)} MedlinePlus documents loaded")
...
def chunk_documents(documents):
    ...
    if page is not None:
        ...
        chunk.metadata["chunk_id"] = f"{doc_name}-c{i}"
...
def main():
    print("=== Clinical RAG Ingestion Pipeline ===\n")

    # Load existing PDF documents
    pdf_documents = load_pdfs(config.DATA_DIR)

    # Load MedlinePlus documents
    medlineplus_documents = load_medlineplus()

    # Combine all documents
    documents = pdf_documents + medlineplus_documents
